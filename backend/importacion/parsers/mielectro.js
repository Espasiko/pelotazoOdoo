/**
 * Parser específico para archivos de MIELECTRO
 * Este módulo contiene la función de parser para procesar datos del proveedor MIELECTRO
 */

import { limpiarPrecio } from '../core/utils.js';

/**
 * Parser para archivos de MIELECTRO
 * @param {Array} datos - Datos a procesar
 * @returns {Object} - Datos procesados y normalizados con categorías detectadas
 */
function parserMIELECTRO(datos) {
  console.log('[parserMIELECTRO] Iniciando procesamiento de datos MIELECTRO');
  ...
  // Mapeo de columnas para MIELECTRO
  const colMappings = {
    codigo: 'REFERENCIA',
    nombre: 'DESCRIPCION',
  ...
  for (let i = 0; i < datos.length; i++) {
    ...
    if (row['__EMPTY'] && !row[colMappings.codigo] && !row[colMappings.nombre]) {
      ...
      console.log(`[parserMIELECTRO] Detectada categoría: ${categoriaActual}`);
      continue;
    }
    ...
    const producto = {
      ...
      proveedor_nombre: 'MIELECTRO',
      nombre_proveedor: 'MIELECTRO',
  ...
  console.log(`[parserMIELECTRO] Procesados ${productos.length} productos y detectadas ${categoriasDetectadas.length} categorías`);

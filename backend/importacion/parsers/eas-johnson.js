/**
 * Parser específico para archivos de EAS-JOHNSON
 * Este módulo contiene la función de parser para procesar datos del proveedor EAS-JOHNSON
 */

import { limpiarPrecio } from '../core/utils.js';

/**
 * Parser para archivos de EAS-JOHNSON
 * @param {Array} datos - Datos a procesar
 * @returns {Object} - Datos procesados y normalizados con categorías detectadas
 */
function parserEASJOHNSON(datos) {
  console.log('[parserEASJOHNSON] Iniciando procesamiento de datos EAS-JOHNSON');
  ...
  // Mapeo de columnas para EAS-JOHNSON
  const colMappings = {
    codigo: 'REFERENCIA',
    nombre: 'DESCRIPCION',
  ...
  for (let i = 0; i < datos.length; i++) {
    ...
    if (row['__EMPTY'] && !row[colMappings.codigo] && !row[colMappings.nombre]) {
      ...
      console.log(`[parserEASJOHNSON] Detectada categoría: ${categoriaActual}`);
      continue;
    }
  ...
  console.log(`[parserEASJOHNSON] Procesados ${productos.length} productos y detectadas ${categoriasDetectadas.length} categorías`);

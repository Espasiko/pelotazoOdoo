/**
 * Parser específico para archivos de NEVIR
 * Este módulo contiene la función de parser para procesar datos del proveedor NEVIR
 */

import { limpiarPrecio } from '../core/utils.js';

/**
 * Parser para archivos de NEVIR
 * @param {Array} datos - Datos a procesar
 * @returns {Object} - Datos procesados y normalizados con categorías detectadas
 */
function parserNEVIR(datos) {
  console.log('[parserNEVIR] Iniciando procesamiento de datos NEVIR');
  ...
  for (let i = 0; i < datos.length; i++) {
    ...
    if (row['__EMPTY'] && !row[colMappings.codigo] && !row[colMappings.nombre]) {
      ...
      console.log(`[parserNEVIR] Detectada categoría: ${categoriaActual}`);
      continue;
    }
  ...
  console.log(`[parserNEVIR] Procesados ${productos.length} productos y detectadas ${categoriasDetectadas.length} categorías`);

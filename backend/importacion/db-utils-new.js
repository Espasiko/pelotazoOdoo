...
/**
 * Crea o actualiza un proveedor en PocketBase
 * @param {string} nombreProveedor - Nombre del proveedor
 * @param {Function} fetchAdminFunc - Función para hacer peticiones a PocketBase
 * @returns {Promise<string>} - ID del proveedor
 */
export async function crearOActualizarProveedor(nombreProveedor, fetchAdminFunc) {
  if (!nombreProveedor) {
    console.warn('[crearOActualizarProveedor] Nombre de proveedor vacío');
    return null;
  }
  ...
  try {
    // 1. Buscar si el proveedor ya existe
    ...
    if (proveedorExistente.items && proveedorExistente.items.length > 0) {
      console.log(`[crearOActualizarProveedor] Proveedor encontrado: ${proveedorExistente.items[0].id} - ${proveedorExistente.items[0].nombre}`);
      return proveedorExistente.items[0].id;
    }
    
    // 2. Si no existe, crearlo
    console.log(`[crearOActualizarProveedor] Creando proveedor: ${nombreNormalizado}`);
    ...
    if (nuevoProveedor && nuevoProveedor.id) {
      console.log(`[crearOActualizarProveedor] Proveedor creado: ${nuevoProveedor.id} - ${nuevoProveedor.nombre}`);
      return nuevoProveedor.id;
    } else {
      console.error('[crearOActualizarProveedor] Error al crear proveedor, respuesta sin ID', nuevoProveedor);
      return null;
    }
  } catch (error) {
    console.error(`[crearOActualizarProveedor] Error:`, error);
    return null;
  }
}
